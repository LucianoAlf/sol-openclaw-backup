-- Draft Fase 2: ambiente controlado. Nao aplicar em producao sem gate SQL.
-- Depende de 001 + 002 e dos helpers canonicos ja usados pelo Caixa:
--   public.sol_caixa_grupo_operacao_ok(uuid,text,text)
--   public.sol_caixa_ator_operacao_ok(uuid,text,text)
-- Continua intent-only: nao cria preview, approval ou movimento financeiro.
...
begin
  ...
  with regras as (
    ...
    select r.*,
      dense_rank() over (order by r.prioridade desc, r.confianca desc) as faixa
    from regras r
  )
  select id, chave_regra, versao, confianca, intencao, matcher,
         campos_obrigatorios, versao_contrato,
         count(*) over ()
  ...
  if v_operacao is null
    or not coalesce(public.sol_caixa_grupo_operacao_ok(v_unidade_id, v_grupo_jid, v_operacao), false)
    or not coalesce(public.sol_caixa_ator_operacao_ok(v_unidade_id, v_actor, v_operacao), false) then
    return jsonb_build_object('ok', true, 'stage', 'deterministic',
      'status', 'manual_review', 'reason', 'canonical_context_rejected',
      'contract_version', 1, 'writes', false);
  end if;
