...
function pareceChamarSol(texto = '') {
  ...
  return /^sol\s*$/.test(n)
    || /(^|[^a-z0-9])@sol\b/.test(n)
    || /(^|[^a-z0-9])sol\s*[,!?:]/.test(n)
    || /^sol\s+(me|nos|voce|vc|pode|poderia|consegue|ve|olha|manda|traz|qual|quais|quanto|quantos|quando|como|onde|porque|por que|preciso|faz|faca|ajuda|verifica|confere|checa|lista|mostra|tem)\b/.test(n)
