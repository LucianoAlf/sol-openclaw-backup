...
const schema = z.object({
  ...
  heartbeatTelegramChatId: z.string().min(1).optional(),
  heartbeatTelegramThreadId: z.coerce.number().optional(),
  heartbeatDryRun: z.coerce.boolean().default(true),
...
export function loadConfig(): Config {
  return schema.parse({
    ...
    heartbeatTelegramChatId: process.env.HEARTBEAT_TELEGRAM_CHAT_ID ?? undefined,
    heartbeatTelegramThreadId: process.env.HEARTBEAT_TELEGRAM_THREAD_ID ?? undefined,
    heartbeatDryRun: process.env.HEARTBEAT_DRY_RUN,
